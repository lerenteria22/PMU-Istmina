...
def aplicar_estilo_plotly(fig, height=350):
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            title_font=dict(color="#2c3e50", size=13),
            automargin=True,
        ),
        yaxis=dict(
            gridcolor="#e5e8e8",
        ...
        legend=dict(
            font=dict(color="#2c3e50", size=11), bgcolor="rgba(255,255,255,0.9)"
        ),
    )
    return fig
...
# ===================================================================
# 2. CARGA Y TRATAMIENTO DE DATOS (EXPORT NATIVO DE GOOGLE SHEETS)
# ===================================================================
def clean_col(name):
...
def check_cond_cols(df, palabras_clave, valores_objetivo):
    cols_coincidentes = [
        c for c in df.columns if any(kw in c for kw in palabras_clave)
    ]
...
def cargar_datos():
    sheet_id = "1UyHaV3J-MJ3lMnQs5lnPGIdGMTn591Tsz1aZ9_E1Z8g"
    url_csv = (
    ...
    try:
        ...
        df_bruto = pd.read_csv(io.StringIO(response.text))
        df_bruto.columns = [clean_col(c) for c in df_bruto.columns]
    except Exception as e:
        st.error(
            f"⚠️ Error de conexión con Google Sheets. Detalle: {str(e)}"
        )
        return pd.DataFrame(), {}
    ...
    def extraer_numero(serie):
        return (
            pd.to_numeric(
                serie.astype(str).str.extract(r"(\d+)")[0], errors="coerce"
            )
            .fillna(0)
            .astype(int)
        )
    ...
    df["barrio_vereda"] = (
        df[col_barrio].fillna("No registrado").astype(str)
        if col_barrio in df.columns
        else "No registrado"
    )
    df["clasificacion_habitabilidad"] = (
        df[col_habitabilidad].fillna("Sin Clasificar").astype(str)
        if col_habitabilidad in df.columns
        else "Sin Clasificar"
    )

    df["total_habitantes"] = (
        extraer_numero(df[col_habitantes]) if col_habitantes in df.columns else 0
    )
    df["ninos"] = extraer_numero(df[col_ninos]) if col_ninos in df.columns else 0
    df["adultos_mayores"] = (
        extraer_numero(df[col_mayores]) if col_mayores in df.columns else 0
    )
    df["n_personas_discapacidad"] = (
        extraer_numero(df[col_discap_num]) if col_discap_num in df.columns else 0
    )
    df["n_mujeres_embarazadas"] = (
        extraer_numero(df[col_emb_num]) if col_emb_num in df.columns else 0
    )

    if col_discap_txt in df.columns:
        mask_txt_discap = (
            df[col_discap_txt].astype(str).str.lower().str.contains("si|sí")
        )
        df.loc[
            mask_txt_discap & (df["n_personas_discapacidad"] == 0),
            "n_personas_discapacidad",
        ] = 1

    if col_emb_txt in df.columns:
        mask_txt_emb = (
            df[col_emb_txt].astype(str).str.lower().str.contains("si|sí")
        )
        df.loc[
            mask_txt_emb & (df["n_mujeres_embarazadas"] == 0),
            "n_mujeres_embarazadas",
        ] = 1

    df["nombre_propietario"] = (
        df[col_propietario].fillna("No registrado").astype(str)
    ...
    if col_fecha_real in df.columns:
        df["fecha_corta"] = (
            pd.to_datetime(df[col_fecha_real], errors="coerce")
            .dt.date.fillna(datetime.now().date())
        )
    else:
    ...
    def estandarizar_barrio(txt):
        ...
        if re.search(r"eduardo|santo|pep", txt):
            return "Eduardo Santos (La Pepé)"
        elif re.search(r"cubi|70|setenta|genoveva|lavander|chambl", txt):
            return "Cubis"
        elif re.search(r"camell|camelo", txt):
            return "Camellón"
        elif re.search(r"comerci|comers", txt):
            return "Comercio"
        elif re.search(r"ofel|offel", txt):
            return "Offel"
        elif re.search(r"independencia|valdes|valdés|valdez", txt):
            return "Independencia"
        elif re.search(r"diego|luis", txt):
            return "Diego Luis"
        elif re.search(r"carretera|carrete|san francisco|francisco", txt):
            return "Carretera (San Francisco)"
        elif re.search(r"agust", txt):
            return "San Agustín"
        elif re.search(r"pueblo|nuevo|meseta", txt):
            return "Pueblo Nuevo"
    ...
    def estandarizar_sector(txt):
        txt = str(txt).lower().strip()
        if "genoveva" in txt:
            return "Santa Genoveva"
        elif "meseta" in txt:
            return "La Meseta"
        elif re.search(r"lavanderia|lavandería|lavander", txt):
            return "Lavandería"
        elif re.search(r"\b70\b|la 70|setenta", txt):
            return "La 70"
        elif re.search(r"antuco|chorro", txt):
            return "Chorro de Antuco"
        elif re.search(r"chamblun|chamblún|chambl", txt):
            return "Chamblún"
    ...
    if col_fecha_real in df.columns:
        ...
        df = df.sort_values(
            by=["nombre_propietario", "barrio_estandar", "fecha_hora_dt"]
        )
        df["minutos_al_siguiente"] = (
            df.groupby(["nombre_propietario", "barrio_estandar"])["fecha_hora_dt"]
            .diff(periods=-1)
            .dt.total_seconds()
            .abs()
            / 60.0
        )
        condicion_mantener = df["minutos_al_siguiente"].isna() | (
            df["minutos_al_siguiente"] > 15
        )
        df = (
            df[condicion_mantener]
            .drop(columns=["fecha_hora_dt", "minutos_al_siguiente"])
    ...
    df["color_riesgo"] = (
        df["clasificacion_habitabilidad"].map(colores_hab).fillna("#95a5a6")
    )
    ...
    df["es_riesgo_estructural"] = check_cond_cols(
        df, ["cimentacion", "columnas", "vigas", "estructurales"], val_criticos
    )
    df["es_afectacion_cubierta"] = check_cond_cols(
        df, ["cubierta", "techo"], val_mod_crit
    )
    df["es_afectacion_muros"] = check_cond_cols(
        df, ["muros", "fachada", "inclinacion"], val_mod_crit
    )
    df["es_colapso"] = check_cond_cols(
        df, ["colaps", "parte_colapsada"], ["Colapso", "Si", "Sí"]
    ) | df["clasificacion_habitabilidad"].astype(str).str.contains(
        "colapso", case=False, na=False
    )
    ...
    df["resumen_danos_criticos"] = (
        df.apply(resumen_danos, axis=1) if cols_sev else "Sin evaluación de daños"
    )
...
with tabs[0]:
    ...
    with c1:
        render_value_box(len(df), "Viviendas Evaluadas", "#2c3e50", "fa-house")
    with c2:
        render_value_box(
            int(
                df["clasificacion_habitabilidad"]
                .str.contains("colapso", case=False, na=False)
                .sum()
            ),
            "Riesgo Inminente (Colapso)",
            "#8e44ad",
            "fa-triangle-exclamation",
        )
    with c3:
        render_value_box(
            int(df["total_habitantes"].sum()),
    ...
    with c4:
        render_value_box(
            int(
                df["ninos"].sum()
                + df["adultos_mayores"].sum()
                + df["n_personas_discapacidad"].sum()
                + df["n_mujeres_embarazadas"].sum()
            ),
    ...
    with g1:
        st.markdown(
            '<span class="chart-title">Estado de Habitabilidad General</span>',
            unsafe_allow_html=True,
        )
    ...
    with g2:
        st.markdown(
            '<span class="chart-title">Habitabilidad Consolidada por Barrio</span>',
            unsafe_allow_html=True,
        )
        df_barrio_cnt = df.groupby("barrio_estandar").size().to_dict()
        df["barrio_etiqueta"] = df["barrio_estandar"].apply(
            lambda x: f"{x} ({df_barrio_cnt.get(x, 0)})"
        )
        df_barrio = (
            df.groupby(["barrio_etiqueta", "clasificacion_habitabilidad"])
            .size()
            .reset_index(name="n")
        )
        fig_barrio = px.bar(
            ...
            orientation="h",
            color_discrete_map=colores_habitabilidad,
            labels={"n": "Cantidad de Viviendas", "barrio_etiqueta": ""},
        )
        fig_barrio = aplicar_estilo_plotly(fig_barrio, height=350)
        fig_barrio.update_layout(
            yaxis={"categoryorder": "total ascending"},
    ...
    st.markdown(
        '<span class="chart-title">Evolución Temporal de Reportes (Línea de'
        ' Tiempo)</span>',
        unsafe_allow_html=True,
    )
...
with tabs[1]:
    ...
    with col_f:
        st.markdown(
            '<span class="chart-title">📍 Filtros Interactivos</span>',
            unsafe_allow_html=True,
        )
        barrio_sel = st.multiselect(
            "Seleccione Barrio / Vereda:",
            options=sorted(df["barrio_estandar"].unique()),
        )
        ...
        estados_filtro = []
        if ch_hab:
            estados_filtro.append("Habitable")
        if ch_rest:
            estados_filtro.append("Habitable con restricciones")
        if ch_noeval:
            estados_filtro.append("No evaluado")
        if ch_nohab:
            estados_filtro.append("No habitable")
        if ch_colapso:
            estados_filtro.append("Riesgo de colapso")

        df_m = df.copy()
        if barrio_sel:
            df_m = df_m[df_m["barrio_estandar"].isin(barrio_sel)]
        if estados_filtro:
            df_m = df_m[df_m["clasificacion_habitabilidad"].isin(estados_filtro)]

        st.markdown(
            '<span class="chart-title">🗺️ Mapa Interactivo</span>',
            unsafe_allow_html=True,
        )
        m = folium.Map(
        ...
        for _, r in df_m.iterrows():
            ...
            doc_str = (
                f"({r['documento']})" if r["documento"] != "No registrado" else ""
            )
            html_popup = f"""
    ...
    with col_m:
        sub_tab1, sub_tab2 = st.tabs(
            ["📋 Padrón Filtrado", "📊 Gráfica Viva Barrios"]
        )
        with sub_tab1:
            st.dataframe(
                df_m[[
            ...
            propietario_sel = st.selectbox(
                "Seleccionar para expediente:",
                options=["-- Seleccionar --"]
                + sorted(df_m["nombre_propietario"].unique().tolist()),
            )
        ...
        with sub_tab2:
            df_viva = (
                df_m.groupby(["barrio_estandar", "clasificacion_habitabilidad"])
                .size()
                .reset_index(name="count")
            )
            barrios_orden = (
                df_viva.groupby("barrio_estandar")["count"]
                .sum()
...
with tabs[2]:
    ...
    with d1:
        render_value_box(
            int(df["es_riesgo_estructural"].sum()),
    ...
    with d2:
        render_value_box(
            int(df["es_afectacion_cubierta"].sum()),
            "Afectación en Cubiertas",
            "#d35400",
            "fa-house",
        )
    with d3:
        render_value_box(
            int(df["es_afectacion_muros"].sum()),
            "Afectación Muros / Fachada",
            "#f39c12",
            "fa-border-all",
        )
    with d4:
        render_value_box(
            int(df["es_colapso"].sum()),
            "Viviendas con Colapso",
            "#8e44ad",
            "fa-circle-exclamation",
        )

    sub_d1, sub_d2 = st.tabs(
        ["🗺️ Mapa de Afectaciones Críticas", "📊 Gráficas de Daño"]
    )
    ...
    with sub_d1:
        ...
        with col_dfiltros:
            ...
            b_dano = st.multiselect(
                "Seleccione Barrio:",
                options=sorted(df["barrio_estandar"].unique()),
                key="bd_dano",
            )
            s_dano = st.multiselect(
                "Seleccione Sector:",
                options=sorted(df["sector_especifico"].unique()),
                key="sd_dano",
            )
            ...
            c_demolicion = st.checkbox("Requiere Demolición Inmediata", key="chk_dem")

            df_mapa_danos = df.copy()

            if b_dano:
                df_mapa_danos = df_mapa_danos[
                    df_mapa_danos["barrio_estandar"].isin(b_dano)
                ]
            if s_dano:
                df_mapa_danos = df_mapa_danos[
                    df_mapa_danos["sector_especifico"].isin(s_dano)
                ]

            if c_cubierta or c_muros or c_estructural or c_colapso or c_demolicion:
                cond_afect = pd.Series(False, index=df_mapa_danos.index)
                if c_cubierta:
                    cond_afect |= df_mapa_danos["es_afectacion_cubierta"]
                if c_muros:
                    cond_afect |= df_mapa_danos["es_afectacion_muros"]
                if c_estructural:
                    cond_afect |= df_mapa_danos["es_riesgo_estructural"]
                if c_colapso:
                    cond_afect |= df_mapa_danos["es_colapso"]
                if c_demolicion:
                    cond_afect |= df_mapa_danos["demolicion_inmediata"]
                df_mapa_danos = df_mapa_danos[cond_afect]
            else:
                df_mapa_danos = df_mapa_danos[
                    df_mapa_danos["es_riesgo_estructural"]
                    | df_mapa_danos["es_afectacion_cubierta"]
                    | df_mapa_danos["es_afectacion_muros"]
                    | df_mapa_danos["es_colapso"]
                    | df_mapa_danos["demolicion_inmediata"]
                ]
        ...
        with col_dmapa:
            ...
            for _, r in df_mapa_danos.iterrows():
                color_point = (
                    "#8e44ad"
                    if r["es_colapso"]
                    else ("#e74c3c" if r["es_riesgo_estructural"] else "#f39c12")
                )
                doc_str = (
                    f"({r['documento']})" if r["documento"] != "No registrado" else ""
                )
                ...
                banner_demolicion = ""
                if r.get("demolicion_inmediata", False):
                    banner_demolicion = "<div style='background-color:#c0392b; color:white; font-size:11px; font-weight:bold; text-align:center; padding:4px; border-radius:4px; margin-bottom:8px; border:1px solid #922b21;'>⚠️ REQUIERE DEMOLICIÓN INMEDIATA</div>"
                ...
                html_p = f"""
                        ...
                        <b>Barrio:</b> {r['barrio_estandar']} ({r['sector_especifico']})<br>
                        <b>Estado de Riesgo:</b> <span style='color:{r['color_riesgo']}; font-weight:bold;'>{r['clasificacion_habitabilidad']}</span><br>
                        <hr style='margin:5px 0;'>
    ...
    with sub_d2:
        ...
        with cd1:
            st.markdown(
                '<span class="chart-title">Severidad por Elemento</span>',
                unsafe_allow_html=True,
            )
            ...
            if cols_d:
                ...
                df_sev = df_sev[
                    df_sev["Nivel"].isin(["Leve", "Moderado", "Severo", "Colapso"])
                ]
                if not df_sev.empty:
                    df_sev["Elemento"] = (
                        df_sev["Elemento"].str.replace("dano_", "").str.title()
                    )
                    colores_dano = {
                        "Leve": "#f1c40f",
                        "Moderado": "#e67e22",
                    ...
                    fig_sev = px.bar(
                        ...
                        color="Nivel",
                        orientation="h",
                        color_discrete_map=colores_dano,
                    )
        ...
        with cd2:
            st.markdown(
                '<span class="chart-title">Señales Visibles Frecuentes</span>',
                unsafe_allow_html=True,
            )
            cols_sn = [
                c
            ...
            if cols_sn:
                ...
                df_sn = df_sn[
                    df_sn["Resp"].astype(str).str.lower().str.contains("si|sí")
                ]
                if not df_sn.empty:
                    df_sn["Alerta"] = df_sn["Alerta"].str.replace("_", " ").str.title()
                    fig_sn = px.bar(
                        df_sn["Alerta"].value_counts().reset_index(name="n"),
                        x="n",
                        y="Alerta",
                        orientation="h",
...
with tabs[3]:
    ...
    with v1:
        render_value_box(
            int(df["ninos"].sum()), "Total Niños y Niñas", color_ninos, "fa-child"
        )
    with v2:
        render_value_box(
            int(df["adultos_mayores"].sum()),
            "Adultos Mayores",
            color_mayores,
            "fa-user",
        )
    with v3:
        render_value_box(
            int(df["n_personas_discapacidad"].sum()),
    ...
    with v4:
        render_value_box(
            int(df["n_mujeres_embarazadas"].sum()),
            "Mujeres Embarazadas",
            color_emb,
            "fa-person-pregnant",
        )

    col_v_izq, col_v_der = st.columns([3, 7])
    with col_v_izq:
        st.markdown(
            '<span class="chart-title">📍 Filtros de Población Vulnerable</span>',
            unsafe_allow_html=True,
        )
        
        b_vuln = st.multiselect(
            "Seleccione Barrio:",
            options=sorted(df["barrio_estandar"].unique()),
            key="bd_vuln",
        )
        ...
        f_pobreza = st.checkbox("Extrema Pobreza", value=True)
        ...
        if not df.empty:
            if f_ninos:
                cond_v |= df["ninos"] > 0
            if f_mayores:
                cond_v |= df["adultos_mayores"] > 0
            if f_discap:
                cond_v |= df["n_personas_discapacidad"] > 0
            if f_emb:
                cond_v |= df["n_mujeres_embarazadas"] > 0
            if f_pobreza:
                cond_v |= df["pobreza_extrema"]

        df_v_mapa = df[cond_v]
        
        if b_vuln:
            df_v_mapa = df_v_mapa[df_v_mapa["barrio_estandar"].isin(b_vuln)]
            
    with col_v_der:
        sub_v1, sub_v2 = st.tabs(
            ["🗺️ Mapa de Vulnerabilidad", "📊 Concentración por Barrio"]
        )
        ...
        with sub_v1:
            ...
            for _, r in df_v_mapa.iterrows():
                total_vuln = int(
                    r["ninos"]
                    + r["adultos_mayores"]
                    + r["n_personas_discapacidad"]
                    + r["n_mujeres_embarazadas"]
                )
                doc_str = (
                    f"({r['documento']})" if r["documento"] != "No registrado" else ""
                )
                ...
                if r["n_mujeres_embarazadas"] > 0:
                    color_marker = color_emb
                elif r["n_personas_discapacidad"] > 0:
                    color_marker = color_discap
                elif r["adultos_mayores"] > 0:
                    color_marker = color_mayores

                banner_pobreza = ""
                if r.get("pobreza_extrema", False):
                    banner_pobreza = "<div style='background-color:#d35400; color:white; font-size:11px; font-weight:bold; text-align:center; padding:4px; border-radius:4px; margin-bottom:8px; border:1px solid #ba4a00;'>⚠️ EXTREMA POBREZA</div>"
        ...
        with sub_v2:
            df_v = (
                df.groupby("barrio_estandar")[
                    ["ninos", "adultos_mayores", "n_personas_discapacidad", "n_mujeres_embarazadas"]
                ]
                .sum()
                .reset_index()
            )
            df_v = df_v.rename(
                columns={
                    ...
                    "n_personas_discapacidad": "Personas con Discapacidad",
                    "n_mujeres_embarazadas": "Mujeres Embarazadas",
                }
            )
            df_v_melt = df_v.melt(
                id_vars=["barrio_estandar"], var_name="Grupo", value_name="Total"
            )
            ...
            fig_v = px.bar(
                ...
                barmode="group",
                orientation="h",
                color_discrete_map=color_map_vuln,
            )
            fig_v = aplicar_estilo_plotly(fig_v, height=450)
            fig_v.update_layout(
                xaxis_title="Cantidad de Personas", yaxis_title="", legend_title_text=""
            )
...
with tabs[4]:
    st.markdown(
        '<span class="chart-title">Matriz de Necesidades Inmediatas'
        " Solicitadas</span>",
        unsafe_allow_html=True,
    )
    df_n = pd.DataFrame({
        ...
        "Total": [
            df["nec_cubierta"].sum(),
            df["nec_materiales"].sum(),
            df["nec_muros"].sum(),
            df["nec_estructura"].sum(),
            df["nec_agua"].sum(),
            df["nec_alojamiento"].sum(),
            df["nec_alimento"].sum(),
        ],
    })
    fig_n = px.bar(
    ...
    fig_n.update_layout(
        showlegend=False, xaxis_title="Viviendas que lo solicitan", yaxis_title=""
    )
    st.plotly_chart(fig_n, use_container_width=True, theme=None)

    st.markdown(
        '<span class="chart-title">🚨 Triage Urgente: Familias Inhabitables con'
        " Población Vulnerable</span>",
        unsafe_allow_html=True,
    )
    df_t = df[
        (
            df["clasificacion_habitabilidad"].str.contains(
                "colapso|no habitable", case=False, na=False
            )
        )
        & (
            (df["ninos"] > 0)
            | (df["adultos_mayores"] > 0)
            | (df["n_personas_discapacidad"] > 0)
            | (df["n_mujeres_embarazadas"] > 0)
        )
    ]

    df_t_show = df_t.rename(
        columns={
            "documento": "Documento",
    ...
    st.dataframe(
        df_t_show[[c for c in cols_triage if c in df_t_show.columns]],
        use_container_width=True,
    )

    if not df_t.empty:
        triage_sel = st.selectbox(
            "Atender caso urgente de:",
            options=["-- Seleccionar --"]
            + sorted(df_t["nombre_propietario"].unique().tolist()),
        )
